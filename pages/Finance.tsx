...
const buildCanonicalFinanceBreakdown = (
  ...
  const cogsFromSales = sales.reduce((sum, t) => roundMoney(sum + normalizeTransactionItems(t.items).reduce((itemSum, item) => itemSum + ((item.buyPrice || 0) * item.quantity), 0)), 0);
  const cogsReversalFromReturns = returns.reduce((sum, t) => roundMoney(sum + normalizeTransactionItems(t.items).reduce((itemSum, item) => itemSum + ((item.buyPrice || 0) * item.quantity), 0)), 0);
...
const getSessionCashTotals = (
  ...
  const supplierPayments = Array.isArray(supplierPaymentsInput) ? supplierPaymentsInput : [];
  ...
  const supplierCashPaymentsFromLedger = supplierPayments.reduce((sum, payment) => {
    ...
    const selectedDate = (payment as any)?.paidAt || (payment as any)?.paymentDate || (payment as any)?.date || (payment as any)?.createdAt || null;
    ...
    let excludedReason = '';
    if (!Number.isFinite(paidAt)) excludedReason = 'invalid_date';
    else if (!isInSession) excludedReason = 'outside_session_window';
    else if (isDeleted) excludedReason = 'deleted';
    else if (!isCash) excludedReason = 'non_cash_method';
...
export default function Finance() {
    ...
    const normalize = (value: unknown): Date | null => {
      ...
      if (typeof value === 'object') {
        ...
        if (typeof anyValue?.seconds === 'number') {
          const ms = anyValue.seconds * 1000 + (typeof anyValue?.nanoseconds === 'number' ? anyValue.nanoseconds / 1_000_000 : 0);
          const d = new Date(ms);
          return Number.isNaN(d.getTime()) ? null : d;
        }
      }
  ...
  const getTxCogs = (tx: Transaction) => normalizeTransactionItems(tx.items).reduce((sum, item) => sum + (resolveBuyPriceForFinanceItem(item, tx.date) * item.quantity), 0);
        ...
        {activeTab === 'cash' && (
          <div className="space-y-4">
            ...
            {activeHistorySession && (() => {
              ...
              let movement: ReturnType<typeof buildShiftCashMovementBreakdown> = { cashInRows: [], cashOutRows: [], cashInTotal: 0, cashOutTotal: 0, expectedCash: roundMoney(activeHistorySession.openingBalance + systemCashTotal) };
              try {
                    ...
                    <div className="space-y-4 p-4 sm:p-6">
                      {activeHistorySession.status === 'closed' && (
                        <div className="flex justify-end">
                          <Button type="button" variant="outline" size="sm" onClick={() => openEditClosingModal(activeHistorySession)}>Edit Closing Amount</Button>
                        </div>
                      )}
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
